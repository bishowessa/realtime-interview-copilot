...
const USAGE_WINDOW_MS: Record<string, number> = {
  "1h": 3_600_000,
  "24h": 86_400_000,
  "7d": 7 * 86_400_000,
  "30d": 30 * 86_400_000,
 ...
 */
function resolveUsageWindow(raw: string | null): { window: string; since: Date } {
  const key = (raw ?? "30d").trim();
  const ms = USAGE_WINDOW_MS[key];
  if (!ms) {
    throw new APIError("BAD_REQUEST", { message: "window must be one of 1h, 24h, 7d, 30d, 90d" });
  }
    ...
    endpoints: {
      ...
      adminGetUser: createAuthEndpoint("/self-hosted-admin/get-user", { method: "GET", use: [sessionMiddleware] }, async (ctx) => {
        ...
        const usageSummary = await getUserUsageSummary(db, userId, usageSince).catch((e) => {
          ...
          return { totals: { events: 0, promptChars: 0, responseChars: 0, durationMs: 0, errors: 0 }, perAction: [] };
        });
      ...
      // ── Usage Analytics ───────────────────────────────────────────
      //
      // All endpoints below accept an optional `?window=` param with one of
      // 1h / 24h / 7d / 30d / 90d (default 30d). Windows are translated to
      // a `since` cutoff and applied uniformly.
      ...
      adminUsageUserDetail: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const [targetUser] = await db
            .select({ id: user.id, email: user.email, name: user.name, createdAt: user.createdAt, lastActiveAt: user.lastActiveAt })
      ...
      adminUsageExportCsv: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          // Hard cap to keep the worker response size bounded.
          const rows = await db
            .select({
              userId: usageEvent.userId,
              userEmail: usageEvent.userEmail,
          ...
          const header = "userId,userEmail,action,flag,model,promptChars,responseChars,durationMs,status,errorCode,createdAt";
          const lines = rows.map((r) =>
            [
              esc(r.userId), esc(r.userEmail), esc(r.action), esc(r.flag), esc(r.model),
              r.promptChars ?? 0, r.responseChars ?? 0, r.durationMs ?? 0,
              esc(r.status), esc(r.errorCode),
